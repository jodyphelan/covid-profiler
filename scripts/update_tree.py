...
def main(args):
    ...
    c.execute("CREATE TABLE mutations (position INT, origins INT, branches TEXT, %s )" % (",".join(["%s TEXT" % s for s in samples])))

    for row in csv.DictReader(open("%s/covid_public.mutation_summary.csv" % args.dir)):
        c.execute("INSERT INTO mutations (position, origins, branches) VALUES (%(position)s,%(origins)s,'%(branches)s')" % row)
        for s in samples:
